% Resolución de Ecuaciones Diferenciales (ED) por medio de Runge-Kutta Euler modificado
...
disp('Resolucion de ED por medio de Euler Modificado')
...
for i=2:N+1
	mED(i,1) = mED(i-1,1) + h;
	wiI = mED(i-1,2) + h*subs(F1,[t,y],[mED(i-1,1),mED(i-1,2)]);
	fun1 = subs(F1,[t,y],[mED(i-1,1),mED(i-1,2)]);
	fun2 = subs(F1,[t,y],[mED(i,1),wiI]);
	mED(i,2) = mED(i-1,2) + (h/2)*fun1 + (h/2)*fun2;
