% Resolución de Ecuaciones Diferenciales (ED) por medio de Runge-Kutta de orden 4
...
disp('Resolucion de ED por medio de Runge-Kutta de orden 4')
...
for i=2:N
    ...
    K2 = h*subs(F1,[t,y],[mED(i-1,1) + (h/2),mED(i-1,2) + (1/2)*K1]);
    K3 = h*subs(F1,[t,y],[mED(i-1,1) + (h/2),mED(i-1,2) + (1/2)*K2]);
    K4 = h*subs(F1,[t,y],[mED(i,1),mED(i-1,2) + K3]);
    ...
    mED(i,1) = mED(i-1,1) + h;
    mED(i,2) = mED(i-1,2) + (1/6)*(K1 + 2*K2 + 2*K3 + K4);
