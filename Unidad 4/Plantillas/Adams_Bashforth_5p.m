% Resolución de Ecuaciones Diferenciales (ED) por medio de Adams-Bashforth de 5 pasos
...
disp('Resolucion de ED por medio de Adams-Bashforth de 5 pasos')
...
for i=6:N
    ...
    mED(i,1) = mED(i-1,1) + h;
    fun = 1901*subs(F1,[t,y],[mED(i-1,1),mED(i-1,2)]) - 2774*subs(F1,[t,y],[mED(i-2,1),mED(i-1,2)]) + 2616*subs(F1,[t,y],[mED(i-3,1),mED(i-3,2)]);
    fun = fun - 1274*subs(F1,[t,y],[mED(i-4,1),mED(i-4,2)]) + 251*subs(F1,[t,y],[mED(i-5,1),mED(i-5,2)]); %se coloca de este modo por motivos de espacio
    mED(i,2) = mED(i-1,2) + (h/720)*fun;
    mED(i,3) = subs(f,mED(i,1));
    mED(i,4) = abs(mED(i,3) - mED(i,2));
    fprintf('%2.15f \t %2.15f \t %2.15f \t %e\n\n', mED(i,1), mED(i,2),mED(i,3),mED(i,4))
end
