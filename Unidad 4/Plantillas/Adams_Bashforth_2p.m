% Resolución de Ecuaciones Diferenciales (ED) por medio de Adams-Bashforth de 2 pasos
...
disp('Resolucion de ED por medio de Adams-Bashforth de 2 pasos')
...
%fprintf('t \t\t\t Wi+1 \t\t\t Y(t) \t\t\t Error \n')
mED(2,1) = mED(1,1) + h;
mED(2,2) = subs(f,mED(2,1));
mED(2,3) = subs(f,mED(2,1));
mED(2,4) = abs(mED(2,3) - mED(2,2));
...
for i=3:N
    ...
    mED(i,1) = mED(i-1,1) + h;
    fun = 3*subs(F1,[t,y],[mED(i-1,1),mED(i-1,2)]) - subs(F1,[t,y],[mED(i-2,1),mED(i-1,2)]);
    mED(i,2) = mED(i-1,2) + (h/2)*fun;
