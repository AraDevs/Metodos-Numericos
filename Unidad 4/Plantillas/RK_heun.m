% Resolución de Ecuaciones Diferenciales (ED) por medio de Runge-Kutta Heun
...
disp('Resolucion de ED por medio de Heun')
...
% Metodo de Runge-Kutta modificado de Heun
% Wi+1 = Wi + (h/4)*f(ti,wi) + (3h/4)*f(ti+(2h/3),wi+(2h/3)*f(ti,wi))
...
mx(2,1) = cond_i;
mx(3,1) = cond_i;
% Encontrando la solucion de la ED ordinaria con ayuda de la condicion
% inicial
s = dsolve(ed,c);
for j=2:n
    wi_interior = mx(2,j-1)+(2/3)*h*subs(F,[t,y],[mx(1,j-1),mx(2,j-1)]); % »» wi+(2h/3)*f(ti,wi)
    mx(2,j) = mx(2,j-1)+(h/4)*subs(F,[t,y],[mx(1,j-1),mx(2,j-1)])+(3/4)*h*subs(F,[t,y],[mx(1,j-1)+(2/3)*h,wi_interior]);
    mx(3,j) = subs(s,mx(1,j));
end
% Imprimiendo datos
fprintf('\n       ti               Wi+1               y(t)\n');
for j=1:n
    fprintf('%9.15f %9.15f %9.15f\n\n',mx(1,j),mx(2,j),mx(3,j));
end