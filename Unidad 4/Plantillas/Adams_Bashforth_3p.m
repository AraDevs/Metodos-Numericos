% Resolución de Ecuaciones Diferenciales (ED) por medio de Adams-Bashforth de 3 pasos
...
disp('Resolucion de ED por medio de Adams-Bashforth de 3 pasos')
...
for i=4:N
    ...
    mED(i,1) = mED(i-1,1) + h;
    fun = 23*subs(F1,[t,y],[mED(i-1,1),mED(i-1,2)]) - 16*subs(F1,[t,y],[mED(i-2,1),mED(i-1,2)]) + 5*subs(F1,[t,y],[mED(i-3,1),mED(i-3,2)]);
    mED(i,2) = mED(i-1,2) + (h/12)*fun;
    mED(i,3) = subs(f,mED(i,1));
    mED(i,4) = abs(mED(i,3) - mED(i,2));
    fprintf('%2.15f \t %2.15f \t %2.15f \t %e\n\n', mED(i,1), mED(i,2),mED(i,3),mED(i,4))
end
